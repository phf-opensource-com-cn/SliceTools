...
from object import UnicObject


class UnicScan(UnicObject):
    def travers(self, path):
        case_set = set()
        for root, dirs, files in os.walk(path):
            for file in files:
                [fname, fename] = os.path.splitext(file)
                if fename.upper() == '.TMAP' and not fname.startswith('_'):
    ...
    def writer(self, cases):
                ...
                if case_id in cases:
                    if exist_value and type_value:
                        if str(self._exist) not in str(exist_value).split('/'):
                            ...
                            ws['{}{}'.format(type_id, row_id)] = '{}/{}'.format(type_value, '优纳')
                        else:
                            pass
                    else:
                        ws['{}{}'.format(exist_id, row_id)] = self._exist
                        ws['{}{}'.format(type_id, row_id)] = '优纳'
        ...
        # 将该列所有旧数据清空
        for cell in new_ws['{}'.format(scol)]:
            cell.value = None

        # 写入文件存在但是表中没有诊断信息的病理号
